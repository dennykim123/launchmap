...
import { gameCategories, getGameCategoryBySlug } from "@/data/game-categories";
...
export async function generateMetadata({
  ...
  if (!category) return {};
  return {
    title: `${category.emoji} ${category.title} - LaunchMap 게임`,
...
export default async function GameCategoryPage({
  ...
  const currentIndex = gameCategories.findIndex((c) => c.slug === slug);
  const prev = currentIndex > 0 ? gameCategories[currentIndex - 1] : null;
  const next =
    currentIndex < gameCategories.length - 1
      ? gameCategories[currentIndex + 1]
      : null;
